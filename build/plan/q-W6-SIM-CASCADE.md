# Proposed docs/15 entry — W6-SIM-CASCADE (the wipe's culprit, and the cascade's parent)

One row. The NUMBER is canon's (doc 01 §6.1: "an extra −4 to the raider whose mistake triggered
the wipe"); the RULE for "whose mistake triggered the wipe" is nowhere in canon and is this unit's,
taken at LOOP-12's recommended default (ship plan §6). The same row records the one ambiguity in
doc 07 §6's cascade rule (SIM-05: which live token is the parent when several are live), taken at
SIM-05's recommended default. **The number is not claimed** — W6-LEDGER owns docs/15 this wave and
assigns the next free id; `handoff-W6-SIM-CASCADE.md` §1 carries the same text as an insertion
before "## Related documents" with `BL-nnn` to be replaced. Nothing in `sim/` cites the row by
number: `RaidSim.WIPE_CULPRIT_DELTA`'s comment says "the BL row handed to W6-LEDGER" and should be
rewritten to the id once it lands.
